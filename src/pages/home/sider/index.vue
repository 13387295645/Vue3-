<template>
  <div class='sider'>
    <!-- 常见科室结构 -->
    <div class="department">
      <div class="header">
        <div class="left">
          <svg t="1728390798897" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            p-id="7939" width="16" height="16">
            <path
              d="M400.696008 129.093147c-17.248849 0-31.233352 13.984503-31.233352 31.233352l0 30.470989c0 17.248849 13.984503 31.233352 31.233352 31.233352s31.233352-13.984503 31.233352-31.233352l0-30.470989C431.92936 143.078673 417.944857 129.093147 400.696008 129.093147z"
              fill="#13227a" p-id="7940"></path>
            <path
              d="M623.647823 129.093147c-17.248849 0-31.233352 13.984503-31.233352 31.233352l0 30.470989c0 17.248849 13.985526 31.233352 31.233352 31.233352 17.248849 0 31.233352-13.984503 31.233352-31.233352l0-30.470989C654.881175 143.078673 640.896672 129.093147 623.647823 129.093147z"
              fill="#13227a" p-id="7941"></path>
            <path
              d="M425.695379 312.937269c11.209296 18.047028 41.974997 48.588625 86.152149 48.588625 43.958164 0 75.100442-30.308283 86.573751-48.223305 9.302877-14.528901 5.068436-33.846876-9.455349-43.149752-14.539134-9.307993-33.851992-5.063319-43.149752 9.455349-0.121773 0.198521-13.379729 19.449981-33.968649 19.449981-19.993357 0-32.428573-18.107403-33.271778-19.373233-9.17087-14.417361-28.28009-18.799158-42.829458-9.760295C421.089477 279.028994 416.591023 298.28557 425.695379 312.937269z"
              fill="#13227a" p-id="7942"></path>
            <path
              d="M564.242851 625.945145l-20.278859 0L543.963992 462.486306c0-17.253966-13.985526-31.233352-31.233352-31.233352-17.248849 0-31.233352 13.979386-31.233352 31.233352l0 163.457816-20.283975 0c-45.924959 0-83.289961 37.363979-83.289961 83.289961l0 103.024421c0 45.924959 37.363979 83.289961 83.289961 83.289961l103.029538 0c45.924959 0 83.289961-37.363979 83.289961-83.289961L647.532813 709.234083C647.532813 663.309124 610.168834 625.945145 564.242851 625.945145zM585.066109 812.258505c0 11.286044-9.537214 20.822235-20.822235 20.822235L461.214337 833.080739c-11.286044 0-20.822235-9.537214-20.822235-20.822235L440.392102 709.234083c0-11.286044 9.537214-20.822235 20.822235-20.822235l103.029538 0c11.286044 0 20.822235 9.537214 20.822235 20.822235L585.066109 812.258505z"
              fill="#13227a" p-id="7943"></path>
            <path
              d="M250.808256 625.945145l-17.482163 0 0-266.970354c0-35.483142 28.864398-64.35368 64.343447-64.35368 17.248849 0 31.233352-13.984503 31.233352-31.233352s-13.985526-31.233352-31.233352-31.233352c-69.924559 0-126.810151 56.890708-126.810151 126.820384l0 266.970354-23.079648 0c-45.924959 0-83.289961 37.363979-83.289961 83.289961l0 103.024421c0 45.924959 37.363979 83.289961 83.289961 83.289961l103.029538 0c45.924959 0 83.289961-37.363979 83.289961-83.289961L334.099241 709.234083C334.098217 663.309124 296.734238 625.945145 250.808256 625.945145zM271.630491 812.258505c0 11.286044-9.537214 20.822235-20.822235 20.822235L147.778718 833.080739c-11.286044 0-20.822235-9.537214-20.822235-20.822235L126.956484 709.234083c0-11.286044 9.537214-20.822235 20.822235-20.822235l103.029538 0c11.286044 0 20.822235 9.537214 20.822235 20.822235L271.630491 812.258505z"
              fill="#13227a" p-id="7944"></path>
            <path
              d="M876.565113 625.945145l-21.961174 0 0-266.970354c0-69.929676-56.890708-126.820384-126.815267-126.820384-17.248849 0-31.233352 13.985526-31.233352 31.233352s13.984503 31.233352 31.233352 31.233352c35.483142 0 64.348564 28.869514 64.348564 64.35368l0 266.970354-18.605753 0c-45.924959 0-83.289961 37.363979-83.289961 83.289961l0 103.024421c0 45.924959 37.363979 83.289961 83.289961 83.289961l103.034655 0c45.924959 0 83.289961-37.363979 83.289961-83.289961L959.856098 709.234083C959.854051 663.309124 922.490072 625.945145 876.565113 625.945145zM897.387347 812.258505c0 11.286044-9.537214 20.822235-20.822235 20.822235L773.530458 833.080739c-11.286044 0-20.822235-9.537214-20.822235-20.822235L752.708224 709.234083c0-11.286044 9.537214-20.822235 20.822235-20.822235l103.034655 0c11.286044 0 20.822235 9.537214 20.822235 20.822235L897.387347 812.258505z"
              fill="#13227a" p-id="7945"></path>
          </svg>
          <span>常见科室</span>
        </div>
        <div class="right">
          <span>全部</span>
          <svg t="1728390754204" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            p-id="6782" width="16" height="16">
            <path
              d="M267.349333 97.450667a40.021333 40.021333 0 0 1 56.490667-2.986667L747.52 475.733333a39.936 39.936 0 0 1 21.333333 34.304l0.085334 2.133334a39.253333 39.253333 0 0 1-21.418667 36.352l-423.68 381.269333a40.021333 40.021333 0 1 1-53.589333-59.392L668.16 512.170667 270.250667 153.941333a40.021333 40.021333 0 0 1-2.901334-56.490666z"
              fill="#A3A3A3" p-id="6783"></path>
          </svg>
        </div>
      </div>
      <div class="content">
        <ul>
          <li>关爱义诊</li>
          <li>全科门诊</li>
          <li>内科</li>
          <li>外科</li>
          <li>名医门诊</li>
          <li>骨外科</li>
          <li>妇科</li>
          <li>老年病科</li>
          <li>儿科</li>
          <li>耳鼻喉科</li>
          <li>神经内科</li>
          <li>口腔科</li>
        </ul>
      </div>
    </div>
    <!-- 平台公告 -->
    <div class="notice">
      <div class="header">
        <div class="left">
          <svg t="1728391132790" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            p-id="9145" width="16" height="16">
            <path d="M342 391.1h24.7v180.4H342z" fill="#FFFFFF" p-id="9146"></path>
            <path
              d="M404.6 711.5c-0.4 3.7-0.6 7.5-0.6 11.2v70c0 59.7 48.4 108 108 108 59.7 0 108-48.4 108-108v-70c0-3.8-0.2-7.5-0.6-11.2H404.6z m0 0"
              fill="#CCE5F6" p-id="9147"></path>
            <path
              d="M512 923.9c-72.3 0-131.1-58.8-131.1-131.1v-70c0-4.5 0.2-9.1 0.7-13.6 1.2-11.8 11.1-20.7 23-20.7h214.9c11.8 0 21.8 8.9 23 20.7 0.5 4.5 0.7 9.1 0.7 13.6v70c-0.1 72.3-58.9 131.1-131.2 131.1z m-84.9-189.3v58.1c0 46.8 38.1 84.9 84.9 84.9 46.8 0 84.9-38.1 84.9-84.9v-58.1H427.1z m148-477.2c-0.8 0-1.6 0-2.4-0.1-12.7-1.3-21.9-12.7-20.6-25.4 0.1-1.4 0.2-2.8 0.2-4.2v-41.1c0-22.2-18.1-40.3-40.3-40.3-22.2 0-40.3 18.1-40.3 40.3v41.1c0 1.4 0.1 2.8 0.2 4.2 1.3 12.7-7.9 24.1-20.6 25.4-12.7 1.3-24.1-7.9-25.4-20.6-0.3-3-0.5-6-0.5-9v-41.1c0-47.7 38.8-86.5 86.5-86.5s86.5 38.8 86.5 86.5v41.1c0 3-0.2 6-0.5 9-1.1 11.9-11.1 20.7-22.8 20.7z m0 0"
              fill="#2197EF" p-id="9148"></path>
            <path
              d="M756.7 616.9c-12.8 0-23.1-10.3-23.1-23.1V481.3c0-122.2-99.4-221.6-221.6-221.6-122.2 0-221.6 99.4-221.6 221.6v112.5c0 12.8-10.3 23.1-23.1 23.1s-23.1-10.3-23.1-23.1V481.3c0-147.7 120.2-267.8 267.8-267.8 147.7 0 267.9 120.2 267.9 267.8v112.5c0 12.8-10.4 23.1-23.2 23.1z m0 0"
              fill="#2197EF" p-id="9149"></path>
            <path d="M168.3 677h687.5v113.6H168.3z" fill="#CCE5F6" p-id="9150"></path>
            <path
              d="M855.7 813.7H168.3c-12.8 0-23.1-10.3-23.1-23.1V677c0-12.8 10.3-23.1 23.1-23.1h687.5c12.8 0 23.1 10.4 23.1 23.1v113.6c0 12.8-10.4 23.1-23.2 23.1z m-664.3-46.2h641.3v-67.4H191.4v67.4z m0 0"
              fill="#2197EF" p-id="9151"></path>
          </svg>
          <span>平台公告</span>
        </div>
        <div class="right">
          <span>全部</span>
          <svg t="1728390754204" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            p-id="6782" width="16" height="16">
            <path
              d="M267.349333 97.450667a40.021333 40.021333 0 0 1 56.490667-2.986667L747.52 475.733333a39.936 39.936 0 0 1 21.333333 34.304l0.085334 2.133334a39.253333 39.253333 0 0 1-21.418667 36.352l-423.68 381.269333a40.021333 40.021333 0 1 1-53.589333-59.392L668.16 512.170667 270.250667 153.941333a40.021333 40.021333 0 0 1-2.901334-56.490666z"
              fill="#A3A3A3" p-id="6783"></path>
          </svg>
        </div>
      </div>
      <div class="content">
        <ul>
          <li>关于延长北京大学国际医院放假的通知</li>
          <li>北京中医药大学东方医院部分科室医生开会通知</li>
          <li>武警总医院号源暂停更新通知</li>
          <li>北京大学人民医院设备/服务院内采购公告</li>
        </ul>
      </div>
    </div>
    <!-- 专题报道 -->
    <div class="report">
      <div class="header">
        <div class="left">
          <svg t="1728392059963" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            p-id="13070" width="16" height="16">
            <path
              d="M82.8416 260.864c7.5264 7.3728 12.8 23.5008 10.9568 33.8432L76.8 394.2912c-2.2016 12.8512 1.6896 20.736 5.3248 25.088a23.04 23.04 0 0 0 18.0224 8.0896c5.1712 0 10.6496-1.536 16.2304-4.4032l89.3952-47.0528a39.936 39.936 0 0 1 17.8176-3.6352 39.936 39.936 0 0 1 17.7664 3.6352l89.4464 47.0016a35.0208 35.0208 0 0 0 16.2304 4.4544 23.04 23.04 0 0 0 17.9712-8.0896c3.6864-4.352 7.5776-12.2368 5.376-25.088l-17.1008-99.584a44.9536 44.9536 0 0 1 11.008-33.792l72.3456-70.5536c9.0112-8.8064 12.4416-19.3024 9.3696-28.7744-3.072-9.472-12.032-15.9232-24.4736-17.7664l-99.9936-14.5408a44.9536 44.9536 0 0 1-28.7744-20.8896L247.9616 17.7664C242.3808 6.5536 233.472 0 223.5392 0c-9.984 0-18.8928 6.5024-24.4736 17.8176l-44.6976 90.5728a44.9536 44.9536 0 0 1-28.7744 20.8896L25.6 143.872c-12.4928 1.8432-21.4016 8.2944-24.4736 17.7664-3.072 9.472 0.3072 19.968 9.3696 28.7744l72.3456 70.5024z m338.6368 455.68l-99.9936-14.5408a44.9536 44.9536 0 0 1-28.7744-20.8896l-44.7488-90.624c-5.5808-11.264-14.4896-17.7664-24.4224-17.7664-9.984 0-18.8928 6.5024-24.4736 17.8176l-44.6976 90.624a44.9024 44.9024 0 0 1-28.7744 20.8384L25.6 716.544c-12.4928 1.8432-21.4016 8.2944-24.4736 17.7664-3.072 9.472 0.3072 19.968 9.3696 28.7744l72.3456 70.5024c7.5264 7.3728 12.8 23.5008 10.9568 33.8432L76.8 967.0144c-2.2016 12.8512 1.6896 20.736 5.3248 25.088a23.04 23.04 0 0 0 18.0224 8.0896 35.0208 35.0208 0 0 0 16.2304-4.4032l89.3952-47.0528a39.936 39.936 0 0 1 17.8176-3.6352c6.8608 0 13.312 1.3312 17.7664 3.6352l89.4464 47.0528a35.0208 35.0208 0 0 0 16.2304 4.4032 23.04 23.04 0 0 0 17.9712-8.0896c3.6864-4.352 7.5776-12.2368 5.376-25.088l-17.1008-99.584a44.9536 44.9536 0 0 1 11.008-33.792l72.3456-70.5536c9.0112-8.8064 12.4416-19.3024 9.3696-28.7744-3.072-9.472-12.032-15.9232-24.4736-17.7664z m129.8944-500.3776h432.896a39.7312 39.7312 0 0 0 0-79.4624h-432.896a39.7312 39.7312 0 0 0 0 79.4624z m432.896 90.0608h-432.896a39.7312 39.7312 0 1 0 0 79.4624h432.896a39.7312 39.7312 0 0 0 0-79.4624z m0 384.4608h-432.896a39.7312 39.7312 0 0 0 0 79.4624h432.896a39.7312 39.7312 0 1 0 0-79.4624z m0 169.472h-432.896a39.7312 39.7312 0 1 0 0 79.4624h432.896a39.7312 39.7312 0 1 0 0-79.4624z"
              fill="#E73208" p-id="13071"></path>
          </svg>
          <span>专题报道</span>
        </div>
        <div class="right">
          <span>全部</span>
          <svg t="1728390754204" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            p-id="6782" width="16" height="16">
            <path
              d="M267.349333 97.450667a40.021333 40.021333 0 0 1 56.490667-2.986667L747.52 475.733333a39.936 39.936 0 0 1 21.333333 34.304l0.085334 2.133334a39.253333 39.253333 0 0 1-21.418667 36.352l-423.68 381.269333a40.021333 40.021333 0 1 1-53.589333-59.392L668.16 512.170667 270.250667 153.941333a40.021333 40.021333 0 0 1-2.901334-56.490666z"
              fill="#A3A3A3" p-id="6783"></path>
          </svg>
        </div>
      </div>
      <div class="content">
        <ul>
          <li>学习贯彻习近平新时代中国特色社会主义思想主题教育</li>
          <li>学习贯彻党的二十大精神</li>
          <li>“四满意”专题</li>
          <li>北京大学人民医院-建院105周年 高质量发展年专题</li>
        </ul>
      </div>
    </div>
  </div>
</template>

<script lang='ts' setup name='Sider'>

//数据

//方法

</script>

<style scoped lang="scss">
.sider {
  color: #7f7f7f;

  .department {
    .header {
      display: flex;
      justify-content: space-between;

      .left {
        display: flex;

        span {
          margin-left: 5px;
        }
      }

      .right {
        display: flex;

        span {
          margin-left: 5px;
        }
      }
    }

    .content {
      ul {
        display: flex;
        flex-wrap: wrap;
        justify-content: space-between;

        li {
          width: 40%;
          margin-top: 20px;
        }
      }
    }
  }

  .notice {
    margin: 80px 0;

    .header {
      display: flex;
      justify-content: space-between;

      .left {
        display: flex;

        span {
          margin-left: 5px;
        }
      }

      .right {
        display: flex;

        span {
          margin-left: 5px;
        }
      }
    }

    .content {
      ul {
        li {
          overflow: hidden;
          text-overflow: ellipsis;
          white-space: nowrap;
          margin-top: 20px;
        }
      }
    }
  }

  .report {
    margin-top: 30px;

    .header {
      display: flex;
      justify-content: space-between;

      .left {
        display: flex;

        span {
          margin-left: 5px;
        }
      }

      .right {
        display: flex;

        span {
          margin-left: 5px;
        }
      }
    }

    .content {
      ul {
        li {
          overflow: hidden;
          text-overflow: ellipsis;
          white-space: nowrap;
          margin-top: 20px;
        }
      }
    }
  }
}
</style>